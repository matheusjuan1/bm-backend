...
class Dash {
    ...
    allHistoryByFK (idfunc) {
        const sql = `SELECT nome,idhistorico,urls,datahora,titulo FROM historico,maquina,funcionario where fkmaquina = idmaquina and fkfuncionario= idfuncionario and idfuncionario= ${idfunc} ORDER BY datahora DESC`;
        return db.query(sql);
    }
}
